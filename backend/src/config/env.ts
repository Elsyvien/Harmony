...
const envSchema = z
  .object({
    ...
    SFU_PROVIDER: z.enum(['mediasoup', 'cloudflare']).default('mediasoup'),
    SFU_AUDIO_ONLY: envBoolean.default(true),
    CLOUDFLARE_SFU_APP_ID: z.string().default(''),
    CLOUDFLARE_SFU_APP_SECRET: z.string().default(''),
    CLOUDFLARE_SFU_ACCOUNT_ID: z.string().default(''),
    CLOUDFLARE_SFU_API_BASE_URL: z.string().url().default('https://rtc.live.cloudflare.com/v1'),
